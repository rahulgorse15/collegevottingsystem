...
%>
    ...
    <tr><th>Candidate  Id</th><th>Candidate Name</th><th>photo</th><th>Total Vote</th><th>department</th></tr>
  <%
    rs=stm.executeQuery("select candidate.cid,candidate.cname,candidate.photo,result.tvote,max(tvote),result.vid,voter.dept,voter.vid from candidate join result on candidate.cid=result.cid join voter on voter.vid=result.vid where result.eleid="+id+"") ;
                   while(rs.next()){
                       win=rs.getInt(5);
    %>
